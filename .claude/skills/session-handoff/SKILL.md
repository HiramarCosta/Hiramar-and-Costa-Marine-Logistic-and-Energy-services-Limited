...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `C:\Users\intel\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`C:\Users\intel\.claude\projects\<project>\memory\`).
